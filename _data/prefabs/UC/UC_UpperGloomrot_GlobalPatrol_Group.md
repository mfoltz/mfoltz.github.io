---
title: UC_UpperGloomrot_GlobalPatrol_Group
nav_order: 1
parent: UC Prefabs
---

# UC_UpperGloomrot_GlobalPatrol_Group

**Prefab GUID:** `-1260008979`

## Components

- **ProjectM.DestroyData**
  - `DestroyReason: ProjectM.DestroyReason Default`

- **ProjectM.DestroyState**
  - `Value: ProjectM.DestroyStateEnum NotDestroyed`

- **Unity.Transforms.Rotation**
  - `Value: Unity.Mathematics.quaternion quaternion(0f, 0f, 0f, 1f)`

- **Unity.Transforms.Translation**
  - `Value: Unity.Mathematics.float3 float3(0f, 0f, 0f)`

- **Unity.Transforms.LocalToWorld**
  - `Value: Unity.Mathematics.float4x4 float4x4(1f, 0f, 0f, 0f,  0f, 1f, 0f, 0f,  0f, 0f, 1f, 0f,  0f, 0f, 0f, 1f)`

- **Unity.Transforms.LocalTransform**
  - `Position: Unity.Mathematics.float3 float3(0f, 0f, 0f)`
  - `Scale: 1`
  - `Rotation: Unity.Mathematics.quaternion quaternion(0f, 0f, 0f, 1f)`

- **Stunlock.Core.PrefabGUID**
  - `_Value: -1260008979`

- **ProjectM.UnitCompositionGroupEntry**
  - *(No fields)*

- **[0]**
  - `TimeRequirement: DTS_Everyday_AllDay PrefabGuid(482895609)`
  - `Id: ProjectM.UnitCompositionId ProjectM.UnitCompositionId`
  - `Weight: 50`
  - `UnitsStartIndex: 0`
  - `UnitsCount: 5`

- **[1]**
  - `TimeRequirement: DTS_Everyday_AllDay PrefabGuid(482895609)`
  - `Id: ProjectM.UnitCompositionId ProjectM.UnitCompositionId`
  - `Weight: 50`
  - `UnitsStartIndex: 5`
  - `UnitsCount: 5`

- **[2]**
  - `TimeRequirement: DTS_Everyday_AllDay PrefabGuid(482895609)`
  - `Id: ProjectM.UnitCompositionId ProjectM.UnitCompositionId`
  - `Weight: 50`
  - `UnitsStartIndex: 10`
  - `UnitsCount: 5`

- **[3]**
  - `TimeRequirement: DTS_Everyday_AllDay PrefabGuid(482895609)`
  - `Id: ProjectM.UnitCompositionId ProjectM.UnitCompositionId`
  - `Weight: 50`
  - `UnitsStartIndex: 15`
  - `UnitsCount: 5`

- **[4]**
  - `TimeRequirement: DTS_Everyday_AllDay PrefabGuid(482895609)`
  - `Id: ProjectM.UnitCompositionId ProjectM.UnitCompositionId`
  - `Weight: 50`
  - `UnitsStartIndex: 20`
  - `UnitsCount: 5`

- **[5]**
  - `TimeRequirement: DTS_Everyday_AllDay PrefabGuid(482895609)`
  - `Id: ProjectM.UnitCompositionId ProjectM.UnitCompositionId`
  - `Weight: 50`
  - `UnitsStartIndex: 25`
  - `UnitsCount: 5`

- **[6]**
  - `TimeRequirement: DTS_Everyday_AllDay PrefabGuid(482895609)`
  - `Id: ProjectM.UnitCompositionId ProjectM.UnitCompositionId`
  - `Weight: 50`
  - `UnitsStartIndex: 30`
  - `UnitsCount: 5`

- **ProjectM.UnitCompositionGroupUnitEntry**
  - *(No fields)*

- **[0]**
  - `Unit: CHAR_Gloomrot_SpiderTank_LightningRod PrefabGuid(1655577903)`
  - `IsVBloodUnit: False`
  - `CustomVBloodUnit: GUID Not Found`
  - `UnitBaseStatsType: ProjectM.UnitBaseStatsType Normal`

- **[1]**
  - `Unit: CHAR_Gloomrot_Batoon PrefabGuid(-1707267769)`
  - `IsVBloodUnit: False`
  - `CustomVBloodUnit: GUID Not Found`
  - `UnitBaseStatsType: ProjectM.UnitBaseStatsType Trash`

- **[2]**
  - `Unit: CHAR_Gloomrot_Batoon PrefabGuid(-1707267769)`
  - `IsVBloodUnit: False`
  - `CustomVBloodUnit: GUID Not Found`
  - `UnitBaseStatsType: ProjectM.UnitBaseStatsType Trash`

- **[3]**
  - `Unit: CHAR_Gloomrot_Tazer PrefabGuid(674807351)`
  - `IsVBloodUnit: False`
  - `CustomVBloodUnit: GUID Not Found`
  - `UnitBaseStatsType: ProjectM.UnitBaseStatsType Trash`

- **[4]**
  - `Unit: CHAR_Gloomrot_Tazer PrefabGuid(674807351)`
  - `IsVBloodUnit: False`
  - `CustomVBloodUnit: GUID Not Found`
  - `UnitBaseStatsType: ProjectM.UnitBaseStatsType Trash`

- **[5]**
  - `Unit: CHAR_Gloomrot_SpiderTank_LightningRod PrefabGuid(1655577903)`
  - `IsVBloodUnit: False`
  - `CustomVBloodUnit: GUID Not Found`
  - `UnitBaseStatsType: ProjectM.UnitBaseStatsType Normal`

- **[6]**
  - `Unit: CHAR_Gloomrot_Pyro PrefabGuid(-322293503)`
  - `IsVBloodUnit: False`
  - `CustomVBloodUnit: GUID Not Found`
  - `UnitBaseStatsType: ProjectM.UnitBaseStatsType Normal`

- **[7]**
  - `Unit: CHAR_Gloomrot_Batoon PrefabGuid(-1707267769)`
  - `IsVBloodUnit: False`
  - `CustomVBloodUnit: GUID Not Found`
  - `UnitBaseStatsType: ProjectM.UnitBaseStatsType Trash`

- **[8]**
  - `Unit: CHAR_Gloomrot_Batoon PrefabGuid(-1707267769)`
  - `IsVBloodUnit: False`
  - `CustomVBloodUnit: GUID Not Found`
  - `UnitBaseStatsType: ProjectM.UnitBaseStatsType Trash`

- **[9]**
  - `Unit: CHAR_Gloomrot_Batoon PrefabGuid(-1707267769)`
  - `IsVBloodUnit: False`
  - `CustomVBloodUnit: GUID Not Found`
  - `UnitBaseStatsType: ProjectM.UnitBaseStatsType Trash`

- **[10]**
  - `Unit: CHAR_Gloomrot_SpiderTank_LightningRod PrefabGuid(1655577903)`
  - `IsVBloodUnit: False`
  - `CustomVBloodUnit: GUID Not Found`
  - `UnitBaseStatsType: ProjectM.UnitBaseStatsType Normal`

- **[11]**
  - `Unit: CHAR_Gloomrot_Railgunner PrefabGuid(1732477970)`
  - `IsVBloodUnit: False`
  - `CustomVBloodUnit: GUID Not Found`
  - `UnitBaseStatsType: ProjectM.UnitBaseStatsType Trash`

- **[12]**
  - `Unit: CHAR_Gloomrot_Tazer PrefabGuid(674807351)`
  - `IsVBloodUnit: False`
  - `CustomVBloodUnit: GUID Not Found`
  - `UnitBaseStatsType: ProjectM.UnitBaseStatsType Trash`

- **[13]**
  - `Unit: CHAR_Gloomrot_Tazer PrefabGuid(674807351)`
  - `IsVBloodUnit: False`
  - `CustomVBloodUnit: GUID Not Found`
  - `UnitBaseStatsType: ProjectM.UnitBaseStatsType Trash`

- **[14]**
  - `Unit: CHAR_Gloomrot_Tazer PrefabGuid(674807351)`
  - `IsVBloodUnit: False`
  - `CustomVBloodUnit: GUID Not Found`
  - `UnitBaseStatsType: ProjectM.UnitBaseStatsType Trash`

- **[15]**
  - `Unit: CHAR_Gloomrot_SpiderTank_LightningRod PrefabGuid(1655577903)`
  - `IsVBloodUnit: False`
  - `CustomVBloodUnit: GUID Not Found`
  - `UnitBaseStatsType: ProjectM.UnitBaseStatsType Normal`

- **[16]**
  - `Unit: CHAR_Gloomrot_Railgunner PrefabGuid(1732477970)`
  - `IsVBloodUnit: False`
  - `CustomVBloodUnit: GUID Not Found`
  - `UnitBaseStatsType: ProjectM.UnitBaseStatsType Trash`

- **[17]**
  - `Unit: CHAR_Gloomrot_Technician PrefabGuid(820492683)`
  - `IsVBloodUnit: False`
  - `CustomVBloodUnit: GUID Not Found`
  - `UnitBaseStatsType: ProjectM.UnitBaseStatsType Trash`

- **[18]**
  - `Unit: CHAR_Gloomrot_Technician PrefabGuid(820492683)`
  - `IsVBloodUnit: False`
  - `CustomVBloodUnit: GUID Not Found`
  - `UnitBaseStatsType: ProjectM.UnitBaseStatsType Trash`

- **[19]**
  - `Unit: CHAR_Gloomrot_Tazer PrefabGuid(674807351)`
  - `IsVBloodUnit: False`
  - `CustomVBloodUnit: GUID Not Found`
  - `UnitBaseStatsType: ProjectM.UnitBaseStatsType Trash`

- **[20]**
  - `Unit: CHAR_Gloomrot_SpiderTank_LightningRod PrefabGuid(1655577903)`
  - `IsVBloodUnit: False`
  - `CustomVBloodUnit: GUID Not Found`
  - `UnitBaseStatsType: ProjectM.UnitBaseStatsType Normal`

- **[21]**
  - `Unit: CHAR_Gloomrot_Technician PrefabGuid(820492683)`
  - `IsVBloodUnit: False`
  - `CustomVBloodUnit: GUID Not Found`
  - `UnitBaseStatsType: ProjectM.UnitBaseStatsType Trash`

- **[22]**
  - `Unit: CHAR_Gloomrot_Technician PrefabGuid(820492683)`
  - `IsVBloodUnit: False`
  - `CustomVBloodUnit: GUID Not Found`
  - `UnitBaseStatsType: ProjectM.UnitBaseStatsType Trash`

- **[23]**
  - `Unit: CHAR_Gloomrot_Technician PrefabGuid(820492683)`
  - `IsVBloodUnit: False`
  - `CustomVBloodUnit: GUID Not Found`
  - `UnitBaseStatsType: ProjectM.UnitBaseStatsType Trash`

- **[24]**
  - `Unit: CHAR_Gloomrot_Technician PrefabGuid(820492683)`
  - `IsVBloodUnit: False`
  - `CustomVBloodUnit: GUID Not Found`
  - `UnitBaseStatsType: ProjectM.UnitBaseStatsType Trash`

- **[25]**
  - `Unit: CHAR_Gloomrot_SpiderTank_LightningRod PrefabGuid(1655577903)`
  - `IsVBloodUnit: False`
  - `CustomVBloodUnit: GUID Not Found`
  - `UnitBaseStatsType: ProjectM.UnitBaseStatsType Normal`

- **[26]**
  - `Unit: CHAR_Gloomrot_TractorBeamer PrefabGuid(-293507834)`
  - `IsVBloodUnit: False`
  - `CustomVBloodUnit: GUID Not Found`
  - `UnitBaseStatsType: ProjectM.UnitBaseStatsType Normal`

- **[27]**
  - `Unit: CHAR_Gloomrot_Technician PrefabGuid(820492683)`
  - `IsVBloodUnit: False`
  - `CustomVBloodUnit: GUID Not Found`
  - `UnitBaseStatsType: ProjectM.UnitBaseStatsType Trash`

- **[28]**
  - `Unit: CHAR_Gloomrot_Tazer PrefabGuid(674807351)`
  - `IsVBloodUnit: False`
  - `CustomVBloodUnit: GUID Not Found`
  - `UnitBaseStatsType: ProjectM.UnitBaseStatsType Trash`

- **[29]**
  - `Unit: CHAR_Gloomrot_Technician PrefabGuid(820492683)`
  - `IsVBloodUnit: False`
  - `CustomVBloodUnit: GUID Not Found`
  - `UnitBaseStatsType: ProjectM.UnitBaseStatsType Trash`

- **[30]**
  - `Unit: CHAR_Gloomrot_SpiderTank_LightningRod PrefabGuid(1655577903)`
  - `IsVBloodUnit: False`
  - `CustomVBloodUnit: GUID Not Found`
  - `UnitBaseStatsType: ProjectM.UnitBaseStatsType Normal`

- **[31]**
  - `Unit: CHAR_Gloomrot_SentryOfficer PrefabGuid(1401026468)`
  - `IsVBloodUnit: False`
  - `CustomVBloodUnit: GUID Not Found`
  - `UnitBaseStatsType: ProjectM.UnitBaseStatsType Normal`

- **[32]**
  - `Unit: CHAR_Gloomrot_Batoon PrefabGuid(-1707267769)`
  - `IsVBloodUnit: False`
  - `CustomVBloodUnit: GUID Not Found`
  - `UnitBaseStatsType: ProjectM.UnitBaseStatsType Trash`

- **[33]**
  - `Unit: CHAR_Gloomrot_Batoon PrefabGuid(-1707267769)`
  - `IsVBloodUnit: False`
  - `CustomVBloodUnit: GUID Not Found`
  - `UnitBaseStatsType: ProjectM.UnitBaseStatsType Trash`

- **[34]**
  - `Unit: CHAR_Gloomrot_Technician PrefabGuid(820492683)`
  - `IsVBloodUnit: False`
  - `CustomVBloodUnit: GUID Not Found`
  - `UnitBaseStatsType: ProjectM.UnitBaseStatsType Trash`

- **Unity.Entities.SpawnTag**
  - *(No fields)*

- **Unity.Entities.Prefab**
  - *(No fields)*

- **Unity.Entities.Simulate**
  - *(No fields)*
