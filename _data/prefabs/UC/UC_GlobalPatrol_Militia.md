---
title: UC_GlobalPatrol_Militia
nav_order: 1
parent: UC Prefabs
---

# UC_GlobalPatrol_Militia

**Prefab GUID:** `-1744210400`

## Components

- **ProjectM.DestroyData**
  - `DestroyReason: ProjectM.DestroyReason Default`

- **ProjectM.DestroyState**
  - `Value: ProjectM.DestroyStateEnum NotDestroyed`

- **Unity.Transforms.Rotation**
  - `Value: Unity.Mathematics.quaternion quaternion(0f, 0f, 0f, 1f)`

- **Unity.Transforms.Translation**
  - `Value: Unity.Mathematics.float3 float3(0f, 0f, 0f)`

- **Unity.Transforms.LocalToWorld**
  - `Value: Unity.Mathematics.float4x4 float4x4(1f, 0f, 0f, 0f,  0f, 1f, 0f, 0f,  0f, 0f, 1f, 0f,  0f, 0f, 0f, 1f)`

- **Unity.Transforms.LocalTransform**
  - `Position: Unity.Mathematics.float3 float3(0f, 0f, 0f)`
  - `Scale: 1`
  - `Rotation: Unity.Mathematics.quaternion quaternion(0f, 0f, 0f, 1f)`

- **Stunlock.Core.PrefabGUID**
  - `_Value: -1744210400`

- **ProjectM.UnitCompositionGroupEntry**
  - *(No fields)*

- **[0]**
  - `TimeRequirement: DTS_Everyday_Day PrefabGuid(-684975497)`
  - `Id: ProjectM.UnitCompositionId ProjectM.UnitCompositionId`
  - `Weight: 50`
  - `UnitsStartIndex: 0`
  - `UnitsCount: 3`

- **[1]**
  - `TimeRequirement: DTS_Everyday_Day PrefabGuid(-684975497)`
  - `Id: ProjectM.UnitCompositionId ProjectM.UnitCompositionId`
  - `Weight: 50`
  - `UnitsStartIndex: 3`
  - `UnitsCount: 3`

- **[2]**
  - `TimeRequirement: DTS_Everyday_Day PrefabGuid(-684975497)`
  - `Id: ProjectM.UnitCompositionId ProjectM.UnitCompositionId`
  - `Weight: 50`
  - `UnitsStartIndex: 6`
  - `UnitsCount: 3`

- **[3]**
  - `TimeRequirement: DTS_Everyday_Day PrefabGuid(-684975497)`
  - `Id: ProjectM.UnitCompositionId ProjectM.UnitCompositionId`
  - `Weight: 50`
  - `UnitsStartIndex: 9`
  - `UnitsCount: 3`

- **[4]**
  - `TimeRequirement: DTS_Everyday_Day PrefabGuid(-684975497)`
  - `Id: ProjectM.UnitCompositionId ProjectM.UnitCompositionId`
  - `Weight: 50`
  - `UnitsStartIndex: 12`
  - `UnitsCount: 3`

- **[5]**
  - `TimeRequirement: DTS_Everyday_Day PrefabGuid(-684975497)`
  - `Id: ProjectM.UnitCompositionId ProjectM.UnitCompositionId`
  - `Weight: 50`
  - `UnitsStartIndex: 15`
  - `UnitsCount: 3`

- **[6]**
  - `TimeRequirement: DTS_Everyday_Night PrefabGuid(2115694930)`
  - `Id: ProjectM.UnitCompositionId ProjectM.UnitCompositionId`
  - `Weight: 50`
  - `UnitsStartIndex: 18`
  - `UnitsCount: 3`

- **[7]**
  - `TimeRequirement: DTS_Everyday_Night PrefabGuid(2115694930)`
  - `Id: ProjectM.UnitCompositionId ProjectM.UnitCompositionId`
  - `Weight: 50`
  - `UnitsStartIndex: 21`
  - `UnitsCount: 3`

- **[8]**
  - `TimeRequirement: DTS_Everyday_Night PrefabGuid(2115694930)`
  - `Id: ProjectM.UnitCompositionId ProjectM.UnitCompositionId`
  - `Weight: 50`
  - `UnitsStartIndex: 24`
  - `UnitsCount: 3`

- **[9]**
  - `TimeRequirement: DTS_Everyday_Night PrefabGuid(2115694930)`
  - `Id: ProjectM.UnitCompositionId ProjectM.UnitCompositionId`
  - `Weight: 50`
  - `UnitsStartIndex: 27`
  - `UnitsCount: 3`

- **[10]**
  - `TimeRequirement: DTS_Everyday_Night PrefabGuid(2115694930)`
  - `Id: ProjectM.UnitCompositionId ProjectM.UnitCompositionId`
  - `Weight: 50`
  - `UnitsStartIndex: 30`
  - `UnitsCount: 3`

- **[11]**
  - `TimeRequirement: DTS_Everyday_Night PrefabGuid(2115694930)`
  - `Id: ProjectM.UnitCompositionId ProjectM.UnitCompositionId`
  - `Weight: 50`
  - `UnitsStartIndex: 33`
  - `UnitsCount: 3`

- **[12]**
  - `TimeRequirement: DTS_Everyday_Night PrefabGuid(2115694930)`
  - `Id: ProjectM.UnitCompositionId ProjectM.UnitCompositionId`
  - `Weight: 50`
  - `UnitsStartIndex: 36`
  - `UnitsCount: 3`

- **ProjectM.UnitCompositionGroupUnitEntry**
  - *(No fields)*

- **[0]**
  - `Unit: CHAR_Militia_Heavy PrefabGuid(2005508157)`
  - `IsVBloodUnit: False`
  - `CustomVBloodUnit: GUID Not Found`
  - `UnitBaseStatsType: ProjectM.UnitBaseStatsType Normal`

- **[1]**
  - `Unit: CHAR_Militia_Light PrefabGuid(-63435588)`
  - `IsVBloodUnit: False`
  - `CustomVBloodUnit: GUID Not Found`
  - `UnitBaseStatsType: ProjectM.UnitBaseStatsType Trash`

- **[2]**
  - `Unit: CHAR_Militia_Light PrefabGuid(-63435588)`
  - `IsVBloodUnit: False`
  - `CustomVBloodUnit: GUID Not Found`
  - `UnitBaseStatsType: ProjectM.UnitBaseStatsType Trash`

- **[3]**
  - `Unit: CHAR_Militia_Bomber PrefabGuid(847893333)`
  - `IsVBloodUnit: False`
  - `CustomVBloodUnit: GUID Not Found`
  - `UnitBaseStatsType: ProjectM.UnitBaseStatsType Normal`

- **[4]**
  - `Unit: CHAR_Militia_Crossbow PrefabGuid(956965183)`
  - `IsVBloodUnit: False`
  - `CustomVBloodUnit: GUID Not Found`
  - `UnitBaseStatsType: ProjectM.UnitBaseStatsType Trash`

- **[5]**
  - `Unit: CHAR_Militia_Light PrefabGuid(-63435588)`
  - `IsVBloodUnit: False`
  - `CustomVBloodUnit: GUID Not Found`
  - `UnitBaseStatsType: ProjectM.UnitBaseStatsType Trash`

- **[6]**
  - `Unit: CHAR_Militia_Longbowman PrefabGuid(203103783)`
  - `IsVBloodUnit: False`
  - `CustomVBloodUnit: GUID Not Found`
  - `UnitBaseStatsType: ProjectM.UnitBaseStatsType Normal`

- **[7]**
  - `Unit: CHAR_Militia_Light PrefabGuid(-63435588)`
  - `IsVBloodUnit: False`
  - `CustomVBloodUnit: GUID Not Found`
  - `UnitBaseStatsType: ProjectM.UnitBaseStatsType Trash`

- **[8]**
  - `Unit: CHAR_Militia_Crossbow PrefabGuid(956965183)`
  - `IsVBloodUnit: False`
  - `CustomVBloodUnit: GUID Not Found`
  - `UnitBaseStatsType: ProjectM.UnitBaseStatsType Trash`

- **[9]**
  - `Unit: CHAR_Militia_Longbowman PrefabGuid(203103783)`
  - `IsVBloodUnit: False`
  - `CustomVBloodUnit: GUID Not Found`
  - `UnitBaseStatsType: ProjectM.UnitBaseStatsType Normal`

- **[10]**
  - `Unit: CHAR_Militia_Guard PrefabGuid(1730498275)`
  - `IsVBloodUnit: False`
  - `CustomVBloodUnit: GUID Not Found`
  - `UnitBaseStatsType: ProjectM.UnitBaseStatsType Normal`

- **[11]**
  - `Unit: CHAR_Militia_Crossbow PrefabGuid(956965183)`
  - `IsVBloodUnit: False`
  - `CustomVBloodUnit: GUID Not Found`
  - `UnitBaseStatsType: ProjectM.UnitBaseStatsType Trash`

- **[12]**
  - `Unit: CHAR_Militia_Guard PrefabGuid(1730498275)`
  - `IsVBloodUnit: False`
  - `CustomVBloodUnit: GUID Not Found`
  - `UnitBaseStatsType: ProjectM.UnitBaseStatsType Normal`

- **[13]**
  - `Unit: CHAR_Militia_Guard PrefabGuid(1730498275)`
  - `IsVBloodUnit: False`
  - `CustomVBloodUnit: GUID Not Found`
  - `UnitBaseStatsType: ProjectM.UnitBaseStatsType Normal`

- **[14]**
  - `Unit: CHAR_Militia_Crossbow PrefabGuid(956965183)`
  - `IsVBloodUnit: False`
  - `CustomVBloodUnit: GUID Not Found`
  - `UnitBaseStatsType: ProjectM.UnitBaseStatsType Trash`

- **[15]**
  - `Unit: CHAR_Militia_Hound PrefabGuid(-249647316)`
  - `IsVBloodUnit: False`
  - `CustomVBloodUnit: GUID Not Found`
  - `UnitBaseStatsType: ProjectM.UnitBaseStatsType Normal`

- **[16]**
  - `Unit: CHAR_Militia_Guard PrefabGuid(1730498275)`
  - `IsVBloodUnit: False`
  - `CustomVBloodUnit: GUID Not Found`
  - `UnitBaseStatsType: ProjectM.UnitBaseStatsType Normal`

- **[17]**
  - `Unit: CHAR_Militia_Longbowman PrefabGuid(203103783)`
  - `IsVBloodUnit: False`
  - `CustomVBloodUnit: GUID Not Found`
  - `UnitBaseStatsType: ProjectM.UnitBaseStatsType Normal`

- **[18]**
  - `Unit: CHAR_Militia_Heavy PrefabGuid(2005508157)`
  - `IsVBloodUnit: False`
  - `CustomVBloodUnit: GUID Not Found`
  - `UnitBaseStatsType: ProjectM.UnitBaseStatsType Normal`

- **[19]**
  - `Unit: CHAR_Militia_Torchbearer PrefabGuid(37713289)`
  - `IsVBloodUnit: False`
  - `CustomVBloodUnit: GUID Not Found`
  - `UnitBaseStatsType: ProjectM.UnitBaseStatsType Trash`

- **[20]**
  - `Unit: CHAR_Militia_Torchbearer PrefabGuid(37713289)`
  - `IsVBloodUnit: False`
  - `CustomVBloodUnit: GUID Not Found`
  - `UnitBaseStatsType: ProjectM.UnitBaseStatsType Trash`

- **[21]**
  - `Unit: CHAR_Militia_Bomber PrefabGuid(847893333)`
  - `IsVBloodUnit: False`
  - `CustomVBloodUnit: GUID Not Found`
  - `UnitBaseStatsType: ProjectM.UnitBaseStatsType Normal`

- **[22]**
  - `Unit: CHAR_Militia_Torchbearer PrefabGuid(37713289)`
  - `IsVBloodUnit: False`
  - `CustomVBloodUnit: GUID Not Found`
  - `UnitBaseStatsType: ProjectM.UnitBaseStatsType Trash`

- **[23]**
  - `Unit: CHAR_Militia_Crossbow PrefabGuid(956965183)`
  - `IsVBloodUnit: False`
  - `CustomVBloodUnit: GUID Not Found`
  - `UnitBaseStatsType: ProjectM.UnitBaseStatsType Trash`

- **[24]**
  - `Unit: CHAR_Militia_Longbowman PrefabGuid(203103783)`
  - `IsVBloodUnit: False`
  - `CustomVBloodUnit: GUID Not Found`
  - `UnitBaseStatsType: ProjectM.UnitBaseStatsType Normal`

- **[25]**
  - `Unit: CHAR_Militia_Torchbearer PrefabGuid(37713289)`
  - `IsVBloodUnit: False`
  - `CustomVBloodUnit: GUID Not Found`
  - `UnitBaseStatsType: ProjectM.UnitBaseStatsType Trash`

- **[26]**
  - `Unit: CHAR_Militia_Guard PrefabGuid(1730498275)`
  - `IsVBloodUnit: False`
  - `CustomVBloodUnit: GUID Not Found`
  - `UnitBaseStatsType: ProjectM.UnitBaseStatsType Normal`

- **[27]**
  - `Unit: CHAR_Militia_Guard PrefabGuid(1730498275)`
  - `IsVBloodUnit: False`
  - `CustomVBloodUnit: GUID Not Found`
  - `UnitBaseStatsType: ProjectM.UnitBaseStatsType Normal`

- **[28]**
  - `Unit: CHAR_Militia_Torchbearer PrefabGuid(37713289)`
  - `IsVBloodUnit: False`
  - `CustomVBloodUnit: GUID Not Found`
  - `UnitBaseStatsType: ProjectM.UnitBaseStatsType Trash`

- **[29]**
  - `Unit: CHAR_Militia_Guard PrefabGuid(1730498275)`
  - `IsVBloodUnit: False`
  - `CustomVBloodUnit: GUID Not Found`
  - `UnitBaseStatsType: ProjectM.UnitBaseStatsType Normal`

- **[30]**
  - `Unit: CHAR_Militia_Heavy PrefabGuid(2005508157)`
  - `IsVBloodUnit: False`
  - `CustomVBloodUnit: GUID Not Found`
  - `UnitBaseStatsType: ProjectM.UnitBaseStatsType Normal`

- **[31]**
  - `Unit: CHAR_Militia_Crossbow PrefabGuid(956965183)`
  - `IsVBloodUnit: False`
  - `CustomVBloodUnit: GUID Not Found`
  - `UnitBaseStatsType: ProjectM.UnitBaseStatsType Trash`

- **[32]**
  - `Unit: CHAR_Militia_Guard PrefabGuid(1730498275)`
  - `IsVBloodUnit: False`
  - `CustomVBloodUnit: GUID Not Found`
  - `UnitBaseStatsType: ProjectM.UnitBaseStatsType Normal`

- **[33]**
  - `Unit: CHAR_Militia_Light PrefabGuid(-63435588)`
  - `IsVBloodUnit: False`
  - `CustomVBloodUnit: GUID Not Found`
  - `UnitBaseStatsType: ProjectM.UnitBaseStatsType Trash`

- **[34]**
  - `Unit: CHAR_Militia_Light PrefabGuid(-63435588)`
  - `IsVBloodUnit: False`
  - `CustomVBloodUnit: GUID Not Found`
  - `UnitBaseStatsType: ProjectM.UnitBaseStatsType Trash`

- **[35]**
  - `Unit: CHAR_Militia_Torchbearer PrefabGuid(37713289)`
  - `IsVBloodUnit: False`
  - `CustomVBloodUnit: GUID Not Found`
  - `UnitBaseStatsType: ProjectM.UnitBaseStatsType Trash`

- **[36]**
  - `Unit: CHAR_Militia_Crossbow PrefabGuid(956965183)`
  - `IsVBloodUnit: False`
  - `CustomVBloodUnit: GUID Not Found`
  - `UnitBaseStatsType: ProjectM.UnitBaseStatsType Trash`

- **[37]**
  - `Unit: CHAR_Militia_Light PrefabGuid(-63435588)`
  - `IsVBloodUnit: False`
  - `CustomVBloodUnit: GUID Not Found`
  - `UnitBaseStatsType: ProjectM.UnitBaseStatsType Trash`

- **[38]**
  - `Unit: CHAR_Militia_Torchbearer PrefabGuid(37713289)`
  - `IsVBloodUnit: False`
  - `CustomVBloodUnit: GUID Not Found`
  - `UnitBaseStatsType: ProjectM.UnitBaseStatsType Trash`

- **39 total elements but only showing the first 36**
  - *(No fields)*

- **Unity.Entities.SpawnTag**
  - *(No fields)*

- **Unity.Entities.Prefab**
  - *(No fields)*

- **Unity.Entities.Simulate**
  - *(No fields)*
