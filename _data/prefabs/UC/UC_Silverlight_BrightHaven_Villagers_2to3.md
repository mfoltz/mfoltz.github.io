---
title: UC_Silverlight_BrightHaven_Villagers_2to3
nav_order: 1
parent: UC Prefabs
---

# UC_Silverlight_BrightHaven_Villagers_2to3

**Prefab GUID:** `741189306`

## Components

- **ProjectM.DestroyData**
  - `DestroyReason: ProjectM.DestroyReason Default`

- **ProjectM.DestroyState**
  - `Value: ProjectM.DestroyStateEnum NotDestroyed`

- **Unity.Transforms.Rotation**
  - `Value: Unity.Mathematics.quaternion quaternion(0f, 0f, 0f, 1f)`

- **Unity.Transforms.Translation**
  - `Value: Unity.Mathematics.float3 float3(0f, 0f, 0f)`

- **Unity.Transforms.LocalToWorld**
  - `Value: Unity.Mathematics.float4x4 float4x4(1f, 0f, 0f, 0f,  0f, 1f, 0f, 0f,  0f, 0f, 1f, 0f,  0f, 0f, 0f, 1f)`

- **Unity.Transforms.LocalTransform**
  - `Position: Unity.Mathematics.float3 float3(0f, 0f, 0f)`
  - `Scale: 1`
  - `Rotation: Unity.Mathematics.quaternion quaternion(0f, 0f, 0f, 1f)`

- **Stunlock.Core.PrefabGUID**
  - `_Value: 741189306`

- **ProjectM.UnitCompositionGroupEntry**
  - *(No fields)*

- **[0]**
  - `TimeRequirement: GUID Not Found`
  - `Id: ProjectM.UnitCompositionId ProjectM.UnitCompositionId`
  - `Weight: 40`
  - `UnitsStartIndex: 0`
  - `UnitsCount: 2`

- **[1]**
  - `TimeRequirement: GUID Not Found`
  - `Id: ProjectM.UnitCompositionId ProjectM.UnitCompositionId`
  - `Weight: 75`
  - `UnitsStartIndex: 2`
  - `UnitsCount: 2`

- **[2]**
  - `TimeRequirement: GUID Not Found`
  - `Id: ProjectM.UnitCompositionId ProjectM.UnitCompositionId`
  - `Weight: 25`
  - `UnitsStartIndex: 4`
  - `UnitsCount: 2`

- **[3]**
  - `TimeRequirement: GUID Not Found`
  - `Id: ProjectM.UnitCompositionId ProjectM.UnitCompositionId`
  - `Weight: 30`
  - `UnitsStartIndex: 6`
  - `UnitsCount: 3`

- **[4]**
  - `TimeRequirement: GUID Not Found`
  - `Id: ProjectM.UnitCompositionId ProjectM.UnitCompositionId`
  - `Weight: 70`
  - `UnitsStartIndex: 9`
  - `UnitsCount: 3`

- **ProjectM.UnitCompositionGroupUnitEntry**
  - *(No fields)*

- **[0]**
  - `Unit: CHAR_ChurchOfLight_Villager_Female PrefabGuid(-1224027101)`
  - `IsVBloodUnit: False`
  - `CustomVBloodUnit: GUID Not Found`
  - `UnitBaseStatsType: ProjectM.UnitBaseStatsType Trash`

- **[1]**
  - `Unit: CHAR_ChurchOfLight_Villager_Male PrefabGuid(-2025921616)`
  - `IsVBloodUnit: False`
  - `CustomVBloodUnit: GUID Not Found`
  - `UnitBaseStatsType: ProjectM.UnitBaseStatsType Trash`

- **[2]**
  - `Unit: CHAR_ChurchOfLight_Villager_Male PrefabGuid(-2025921616)`
  - `IsVBloodUnit: False`
  - `CustomVBloodUnit: GUID Not Found`
  - `UnitBaseStatsType: ProjectM.UnitBaseStatsType Trash`

- **[3]**
  - `Unit: CHAR_ChurchOfLight_Villager_Male PrefabGuid(-2025921616)`
  - `IsVBloodUnit: False`
  - `CustomVBloodUnit: GUID Not Found`
  - `UnitBaseStatsType: ProjectM.UnitBaseStatsType Trash`

- **[4]**
  - `Unit: CHAR_ChurchOfLight_Villager_Female PrefabGuid(-1224027101)`
  - `IsVBloodUnit: False`
  - `CustomVBloodUnit: GUID Not Found`
  - `UnitBaseStatsType: ProjectM.UnitBaseStatsType Trash`

- **[5]**
  - `Unit: CHAR_ChurchOfLight_Villager_Female PrefabGuid(-1224027101)`
  - `IsVBloodUnit: False`
  - `CustomVBloodUnit: GUID Not Found`
  - `UnitBaseStatsType: ProjectM.UnitBaseStatsType Trash`

- **[6]**
  - `Unit: CHAR_ChurchOfLight_Villager_Female PrefabGuid(-1224027101)`
  - `IsVBloodUnit: False`
  - `CustomVBloodUnit: GUID Not Found`
  - `UnitBaseStatsType: ProjectM.UnitBaseStatsType Trash`

- **[7]**
  - `Unit: CHAR_ChurchOfLight_Villager_Female PrefabGuid(-1224027101)`
  - `IsVBloodUnit: False`
  - `CustomVBloodUnit: GUID Not Found`
  - `UnitBaseStatsType: ProjectM.UnitBaseStatsType Trash`

- **[8]**
  - `Unit: CHAR_ChurchOfLight_Villager_Male PrefabGuid(-2025921616)`
  - `IsVBloodUnit: False`
  - `CustomVBloodUnit: GUID Not Found`
  - `UnitBaseStatsType: ProjectM.UnitBaseStatsType Trash`

- **[9]**
  - `Unit: CHAR_ChurchOfLight_Villager_Female PrefabGuid(-1224027101)`
  - `IsVBloodUnit: False`
  - `CustomVBloodUnit: GUID Not Found`
  - `UnitBaseStatsType: ProjectM.UnitBaseStatsType Trash`

- **[10]**
  - `Unit: CHAR_ChurchOfLight_Villager_Male PrefabGuid(-2025921616)`
  - `IsVBloodUnit: False`
  - `CustomVBloodUnit: GUID Not Found`
  - `UnitBaseStatsType: ProjectM.UnitBaseStatsType Trash`

- **[11]**
  - `Unit: CHAR_ChurchOfLight_Villager_Male PrefabGuid(-2025921616)`
  - `IsVBloodUnit: False`
  - `CustomVBloodUnit: GUID Not Found`
  - `UnitBaseStatsType: ProjectM.UnitBaseStatsType Trash`

- **Unity.Entities.SpawnTag**
  - *(No fields)*

- **Unity.Entities.Prefab**
  - *(No fields)*

- **Unity.Entities.Simulate**
  - *(No fields)*
