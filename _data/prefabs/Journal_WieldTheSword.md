---
title: Journal_WieldTheSword
guid: 436375429
categories: ['All', 'Journal']
nav_exclude: true
search_exclude: false
layout: default
---

# Journal_WieldTheSword

**GUID:** `436375429`

**Categories:** All, Journal

## Components

- **ProjectM.AchievementData**
  - `Level: 10`
  - `Reward: Journal_Reward_Tech_SpellSlot1 PrefabGuid(-2023076386)`
  - `Dependency: GUID Not Found`

- **ProjectM.DestroyData**
  - `DestroyReason: ProjectM.DestroyReason Default`

- **ProjectM.DestroyState**
  - `Value: ProjectM.DestroyStateEnum NotDestroyed`

- **Stunlock.Core.PrefabGUID**
  - `_Value: 436375429`

- **ProjectM.AchievementSubTaskEntry**
  - *(No fields)*

- **[0]**
  - `PrefabGuid: Journal_SubTask_OpenInventory PrefabGuid(-1183631535)`

- **[1]**
  - `PrefabGuid: Journal_SubTask_Craft_BoneSword PrefabGuid(-333019031)`

- **[2]**
  - `PrefabGuid: Journal_SubTask_Craft_BoneRing PrefabGuid(894263396)`

- **[3]**
  - `PrefabGuid: Journal_SubTask_Kill_EnemiesWithSword PrefabGuid(-363083008)`

- **Unity.Entities.SpawnTag**
  - *(No fields)*

- **Unity.Entities.Prefab**
  - *(No fields)*

- **Unity.Entities.Simulate**
  - *(No fields)*

