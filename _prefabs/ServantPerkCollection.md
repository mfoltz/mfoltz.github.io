---
title: ServantPerkCollection
guid: -64904809
categories: ['All', 'Servant']
nav_exclude: true
search_exclude: true
layout: default
---

# ServantPerkCollection

**GUID:** `-64904809`

**Categories:** [All](/prefabs/All), [Servant](/prefabs/Servant)

## Components

- [ProjectM.SingletonPrefab](/components/SingletonPrefab){:target="_blank"}
  - `AutomaticInstantiation: True`
  - `OverrideExisting: False`

- [ProjectM.DestroyData](/components/DestroyData){:target="_blank"}
  - `DestroyReason: ProjectM.DestroyReason Default`

- [ProjectM.DestroyState](/components/DestroyState){:target="_blank"}
  - `Value: ProjectM.DestroyStateEnum NotDestroyed`

- [Unity.Transforms.Rotation](/components/Rotation){:target="_blank"}
  - `Value: Unity.Mathematics.quaternion quaternion(0f, 0f, 0f, 1f)`

- [Unity.Transforms.Translation](/components/Translation){:target="_blank"}
  - `Value: Unity.Mathematics.float3 float3(0f, 0f, 0f)`

- [Unity.Transforms.LocalToWorld](/components/LocalToWorld){:target="_blank"}
  - `Value: Unity.Mathematics.float4x4 float4x4(1f, 0f, 0f, 0f,  0f, 1f, 0f, 0f,  0f, 0f, 1f, 0f,  0f, 0f, 0f, 1f)`

- [Unity.Transforms.LocalTransform](/components/LocalTransform){:target="_blank"}
  - `Position: Unity.Mathematics.float3 float3(0f, 0f, 0f)`
  - `Scale: 1`
  - `Rotation: Unity.Mathematics.quaternion quaternion(0f, 0f, 0f, 1f)`

- [Stunlock.Core.PrefabGUID](/components/PrefabGUID){:target="_blank"}
  - `_Value: -64904809`

- [ProjectM.ServantBloodTypePerkElement](/components/ServantBloodTypePerkElement){:target="_blank"}

- **[0]**
  - `BloodType: BloodType_Rogue PrefabGuid(-1620185637)`
  - `Perk: ServantPerk_BloodType_Rogue PrefabGuid(-1937299872)`

- **[1]**
  - `BloodType: BloodType_Brute PrefabGuid(804798592)`
  - `Perk: ServantPerk_BloodType_Brute PrefabGuid(740985277)`

- **[2]**
  - `BloodType: BloodType_Worker PrefabGuid(-1776904174)`
  - `Perk: ServantPerk_BloodType_Worker PrefabGuid(-49207502)`

- **[3]**
  - `BloodType: BloodType_Warrior PrefabGuid(-516976528)`
  - `Perk: ServantPerk_BloodType_Warrior PrefabGuid(-1947526080)`

- **[4]**
  - `BloodType: BloodType_Scholar PrefabGuid(1476452791)`
  - `Perk: ServantPerk_BloodType_Scholar PrefabGuid(-1410655756)`

- **[5]**
  - `BloodType: BloodType_Draculin PrefabGuid(1328126535)`
  - `Perk: ServantPerk_BloodType_Warrior PrefabGuid(-1947526080)`

- **[6]**
  - `BloodType: BloodType_Mutant PrefabGuid(1821108694)`
  - `Perk: ServantPerk_BloodType_Rogue PrefabGuid(-1937299872)`

- **[7]**
  - `BloodType: BloodType_Corruption PrefabGuid(-1382693416)`
  - `Perk: ServantPerk_BloodType_Scholar PrefabGuid(-1410655756)`

- [ProjectM.ServantFactionPerkElement](/components/ServantFactionPerkElement){:target="_blank"}

- **[0]**
  - `Perk: ServantPerk_Faction_FarbaneExpert PrefabGuid(1427742366)`
  - `Faction: ProjectM.FactionEnum Bandits`

- **[1]**
  - `Perk: ServantPerk_Faction_DunleyExpert PrefabGuid(81754057)`
  - `Faction: ProjectM.FactionEnum Militia`

- **[2]**
  - `Perk: ServantPerk_Faction_CursedExpert PrefabGuid(-1394605411)`
  - `Faction: ProjectM.FactionEnum WerewolvesHuman`

- **[3]**
  - `Perk: ServantPerk_Faction_DunleyExpert PrefabGuid(81754057)`
  - `Faction: ProjectM.FactionEnum Prisoners`

- **[4]**
  - `Perk: ServantPerk_Faction_SilverExpert PrefabGuid(226032448)`
  - `Faction: ProjectM.FactionEnum ChurchOfLum`

- **[5]**
  - `Perk: ServantPerk_Faction_SilverExpert PrefabGuid(226032448)`
  - `Faction: ProjectM.FactionEnum ChurchOfLum`

- **[6]**
  - `Perk: ServantPerk_Faction_SilverExpert PrefabGuid(226032448)`
  - `Faction: ProjectM.FactionEnum ChurchOfLum_Slaves`

- **[7]**
  - `Perk: ServantPerk_Faction_SilverExpert PrefabGuid(226032448)`
  - `Faction: ProjectM.FactionEnum ChurchOfLum_Slaves_Rioters`

- **[8]**
  - `Perk: ServantPerk_Faction_GloomrotExpert PrefabGuid(-1509519094)`
  - `Faction: ProjectM.FactionEnum Gloomrot`

- **[9]**
  - `Perk: ServantPerk_Faction_CursedExpert PrefabGuid(-1394605411)`
  - `Faction: ProjectM.FactionEnum Werewolves`

- **[10]**
  - `Perk: ServantPerk_Faction_NoctemExpert PrefabGuid(-1031211388)`
  - `Faction: ProjectM.FactionEnum Legion`

- **[11]**
  - `Perk: ServantPerk_Faction_OakveilExpert PrefabGuid(-1745352584)`
  - `Faction: ProjectM.FactionEnum Blackfangs`

- **[12]**
  - `Perk: ServantPerk_Faction_GloomrotExpert PrefabGuid(-1509519094)`
  - `Faction: ProjectM.FactionEnum Mutants`

- **[13]**
  - `Perk: ServantPerk_Faction_SilverExpert PrefabGuid(226032448)`
  - `Faction: ProjectM.FactionEnum Harpy`

- **[14]**
  - `Perk: ServantPerk_Faction_CursedExpert PrefabGuid(-1394605411)`
  - `Faction: ProjectM.FactionEnum Cursed`

- **ProjectM.ServantPerkCollectionSingleton isn't handled**
  - *(No fields)*

- [Unity.Entities.SpawnTag](/components/SpawnTag){:target="_blank"}
  - *(No fields)*

- [Unity.Entities.Prefab](/components/Prefab){:target="_blank"}
  - *(No fields)*

- [Unity.Entities.Simulate](/components/Simulate){:target="_blank"}
  - *(No fields)*
