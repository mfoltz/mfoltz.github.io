---
nav_exclude: true
search_exclude: true
---

# TraderItemEntry

```csharp
[StructLayout(2)]
public struct TraderItemEntry
{
	static TraderItemEntry()
	{
		Il2CppClassPointerStore<TraderItemEntry>.NativeClassPtr = IL2CPP.GetIl2CppClass("ProjectM.dll", "ProjectM", "TraderItemEntry");
		IL2CPP.il2cpp_runtime_class_init(Il2CppClassPointerStore<TraderItemEntry>.NativeClassPtr);
		TraderItemEntry.NativeFieldInfoPtr_ItemGuid = IL2CPP.GetIl2CppField(Il2CppClassPointerStore<TraderItemEntry>.NativeClassPtr, "ItemGuid");
	}
	public Object BoxIl2CppObject()
	{
		return new Object(IL2CPP.il2cpp_value_box(Il2CppClassPointerStore<TraderItemEntry>.NativeClassPtr, ref this));
	}
	private static readonly IntPtr NativeFieldInfoPtr_ItemGuid;
	[FieldOffset(0)]
	public PrefabGUID ItemGuid;
}
