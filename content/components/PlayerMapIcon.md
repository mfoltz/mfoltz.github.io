---
nav_exclude: true
search_exclude: true
---

```csharp
public struct PlayerMapIcon
{
	static PlayerMapIcon()
	{
		Il2CppClassPointerStore<PlayerMapIcon>.NativeClassPtr = IL2CPP.GetIl2CppClass("ProjectM.Shared.dll", "ProjectM", "PlayerMapIcon");
		IL2CPP.il2cpp_runtime_class_init(Il2CppClassPointerStore<PlayerMapIcon>.NativeClassPtr);
		PlayerMapIcon.NativeFieldInfoPtr_UserName = IL2CPP.GetIl2CppField(Il2CppClassPointerStore<PlayerMapIcon>.NativeClassPtr, "UserName");
	}
	public Object BoxIl2CppObject()
	{
		return new Object(IL2CPP.il2cpp_value_box(Il2CppClassPointerStore<PlayerMapIcon>.NativeClassPtr, ref this));
	}
	private static readonly IntPtr NativeFieldInfoPtr_UserName;

	public FixedString64Bytes UserName;
}
```
