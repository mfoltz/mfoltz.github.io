---
title: GeneralGameplayCollectionAset
guid: 715471180
categories: ['All', 'Remainders']
nav_exclude: true
search_exclude: true
layout: default
---

**GUID:** `715471180`

**Categories:** [All]({{% relref "prefabs/All.md" %}}), [Remainders]({{% relref "prefabs/Remainders.md" %}})

## Components

- [ProjectM.GeneralGameplayCollection]({{% relref "components/GeneralGameplayCollection.md" %}})
  - `LKey_PerformanceInfoMessage: 8a60b069-13fb-43f6-88c2-f61107f31f34 - We are experiencing technical issues on some hardware setups resulting in performance deterioration over time. If you are experiencing a loss in performance, try reconnecting and/or restarting your client from time to time.`
  - `LKey_ServerIsRestartingMessage: 8d28d155-58b2-4aee-8b95-1efde47c9608 - Server will restart in {timeinminutes} minutes. Try to be in a safe place when this happens!`
  - `LKey_ServerFailedSavingMessage: b1a0a3dd-236a-4df1-9c03-012a0429b806 - The server failed to save the game. Error Type: {error_type}, Error Code: {error_code}. Server administrators can find detailed advanced information in the server log file.`
  - `LKey_AFKWarningMessage: 0e8f454f-98d9-47d8-8296-6c4717ccb869 - Warning! You have been detected as AFK and will be kicked in {timeinminutes} minutes.`
  - `LKey_ServerQAMessage: 6786662e-2309-4f5f-a109-80169a59631c - Bug Announcement - Serious issue detected on the server with ID: '{error_type}'`

- **If you are hosting the server, Stunlock Studios would love the log file for the current server session.If you can also figure out how to reproduce this issue that would be great into to share with us!**
  - `LKey_ClientQAMessage: 8b360126-cfd8-43d6-9dfb-6213ef5f89dc - Bug Announcement - Serious issue detected on your client with ID: '{error_type}'`

- **Stunlock Studios would love to have the log file for your current client session. If you can also figure out how to reproduce this issue that would be great into to share with us!**
  - `TimeLocalizationKeys: ProjectM.UI.TimeLocalizationKeys ProjectM.UI.TimeLocalizationKeys`
  - `DraculaChunk: ProjectM.Terrain.TerrainChunk 24,2`
  - `DraculaChunkVictory: ProjectM.Terrain.TerrainChunk 24,1`
  - `NetworkedWarEventPrefabGuid: TM_WarEvent_Networked PrefabGuid(-1337812210)`
  - `ChatQuips: Unity.Collections.FixedList512Bytes`1[ProjectM.ChatQuip] Unity.Collections.FixedList512Bytes`1[ProjectM.ChatQuip]`
  - `LKey_TimeSinceAwakenedStat: 595ddfbe-a413-4d02-8547-2a1cc766df61 - Time since awoken from crypt: {time}`
  - `LKey_ActiveTimeStat: e01b705b-d8f3-4a47-932d-8ca0b1fca167 - Time Played: {timeplayed}`
  - `LKey_UnitKillsStat: cec4b3bb-d7dc-4123-8915-6fd6380f87fb - Units Killed: {unitkills}`
  - `LKey_VBloodKillsStat: da3673bf-22fb-49f7-885a-2dab9b5036be - Bosses Killed: {vbloodkills}`
  - `LKey_DeathsStat: 6527a3a1-48ba-494a-bdef-d0d74764d47e - Deaths: {deaths}`
  - `LKey_DistanceTravelledStat: 6526fef3-775a-42f4-b69c-db9022b1e1e4 - Distance Travelled: {value} metres`
  - `LKey_BloodConsumedStat: c2ad25d8-c0bd-414e-9e83-e3e88fd4c4f4 - Blood Consumed: {value} litres`

- [Stunlock.Core.PrefabGUID]({{% relref "components/PrefabGUID.md" %}})
  - `_Value: 715471180`

- [Unity.Entities.Prefab]({{% relref "components/Prefab.md" %}})

- [Stunlock.Core.Authoring.ConvertedScriptableObject]({{% relref "components/ConvertedScriptableObject.md" %}})

- [Unity.Entities.Simulate]({{% relref "components/Simulate.md" %}})
