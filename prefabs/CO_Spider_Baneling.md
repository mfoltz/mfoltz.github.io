---
title: CO_Spider_Baneling
guid: 1989202755
categories: ['All', 'CO']
nav_exclude: true
search_exclude: true
layout: default
---

# CO_Spider_Baneling

**GUID:** `1989202755`

**Categories:** [All](/prefabs/All), [CO](/prefabs/CO)

## Components

- [ProjectM.DestroyData](/components/DestroyData)
  - `DestroyReason: ProjectM.DestroyReason Default`

- [ProjectM.DestroyState](/components/DestroyState)
  - `Value: ProjectM.DestroyStateEnum NotDestroyed`

- [ProjectM.Behaviours.CastOptionRoot](/components/CastOptionRoot)
  - `MaxRange: 9`

- [Stunlock.Core.PrefabGUID](/components/PrefabGUID)
  - `_Value: 1989202755`

- [Unity.Entities.LinkedEntityGroup](/components/LinkedEntityGroup)

- **[0]**
  - `Value: Prefab CO_Spider_Baneling PrefabGuid(1989202755) - Entity(62569:1)`

- **[1]**
  - `Value: Entity(37997:4)`

- **[2]**
  - `Value: Entity(11892:5)`

- **[3]**
  - `Value: Entity(11893:5)`

- [ProjectM.Behaviours.CastOptionStateBuffer](/components/CastOptionStateBuffer)

- **[0]**
  - `State: ProjectM.GenericEnemyState Combat`
  - `Entity: Entity(37997:4)`

- [Unity.Entities.SpawnTag](/components/SpawnTag)

- [Unity.Entities.Prefab](/components/Prefab)

- [Unity.Entities.Simulate](/components/Simulate)
