...
export function SectionHeader({ title, subtitle }: { title: string; subtitle?: string }) {
  return (
    <header className="mb-6 max-w-4xl">
      <h1 className="text-[1.65rem] font-semibold leading-tight text-[var(--database-ink)] sm:text-[1.95rem]">{title}</h1>
...
export function ErrorState({ message }: { message: string }) {
  return <div className="rounded-[1rem] border border-[rgba(219,39,119,0.32)] bg-[rgba(219,39,119,0.1)] p-4 text-[var(--database-ink)]">{message}</div>;
}
