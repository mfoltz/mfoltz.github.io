---
title: ProjectM.DropItemSystem
nav_exclude: true
search_exclude: true
---

# ProjectM.DropItemSystem (Server)

### _EventQuery

- **All Components:**
  - [ProjectM.Network.FromCharacter [ReadOnly]](/components/FromCharacter){:target="_blank"}
  - [ProjectM.Network.DropItemAtSlotEvent [ReadOnly]](/components/DropItemAtSlotEvent){:target="_blank"}

### _EventQuery2

- **All Components:**
  - [ProjectM.Network.FromCharacter [ReadOnly]](/components/FromCharacter){:target="_blank"}
  - [ProjectM.Network.DropEquippedItemEvent [ReadOnly]](/components/DropEquippedItemEvent){:target="_blank"}

### _EventQuery3

- **All Components:**
  - [ProjectM.Network.FromCharacter [ReadOnly]](/components/FromCharacter){:target="_blank"}
  - [ProjectM.Network.DropEntireInventoryEvent [ReadOnly]](/components/DropEntireInventoryEvent){:target="_blank"}

### _DropItemImpairedBuffsQuery

- **All Components:**
  - [ProjectM.Buff [ReadOnly]](/components/Buff){:target="_blank"}
  - [ProjectM.DropItemImpairedBuff [ReadOnly]](/components/DropItemImpairedBuff){:target="_blank"}

### __query_1470978519_0

- **All Components:**
  - [ProjectM.Network.FromCharacter [ReadOnly]](/components/FromCharacter){:target="_blank"}
  - [ProjectM.Network.DropItemAtSlotEvent [ReadOnly]](/components/DropItemAtSlotEvent){:target="_blank"}

### __query_1470978519_1

- **All Components:**
  - [ProjectM.Network.FromCharacter [ReadOnly]](/components/FromCharacter){:target="_blank"}
  - [ProjectM.Network.DropEquippedItemEvent [ReadOnly]](/components/DropEquippedItemEvent){:target="_blank"}

### __query_1470978519_2

- **All Components:**
  - [ProjectM.Network.FromCharacter [ReadOnly]](/components/FromCharacter){:target="_blank"}
  - [ProjectM.Network.DropEntireInventoryEvent [ReadOnly]](/components/DropEntireInventoryEvent){:target="_blank"}

### __query_1470978519_3

- **All Components:**
  - [ProjectM.ServerRootPrefabCollection [ReadOnly]](/components/ServerRootPrefabCollection){:target="_blank"}

