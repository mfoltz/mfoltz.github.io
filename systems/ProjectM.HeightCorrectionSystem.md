---
title: ProjectM.HeightCorrectionSystem
nav_exclude: true
search_exclude: true
---

# ProjectM.HeightCorrectionSystem (Server)

### __query_496388984_0

- **All Components:**
  - [ProjectM.Height](/components/Height){:target="_blank"}
  - [Unity.Transforms.Translation [ReadOnly]](/components/Translation){:target="_blank"}
- **None Components:**
  - [ProjectM.Network.IsConnected [ReadOnly]](/components/IsConnected){:target="_blank"}

### __query_496388984_1

- **All Components:**
  - [ProjectM.Height](/components/Height){:target="_blank"}
  - [Unity.Transforms.Translation [ReadOnly]](/components/Translation){:target="_blank"}
  - [ProjectM.Network.IsConnected [ReadOnly]](/components/IsConnected){:target="_blank"}

### __query_496388984_2

- **All Components:**
  - [ProjectM.Height [ReadOnly]](/components/Height){:target="_blank"}
  - [ProjectM.BuffableFlagState [ReadOnly]](/components/BuffableFlagState){:target="_blank"}
  - [Unity.Transforms.Translation](/components/Translation){:target="_blank"}
  - [ProjectM.SnapToHeight [ReadOnly]](/components/SnapToHeight){:target="_blank"}

### __query_496388984_3

- **All Components:**
  - [ProjectM.Height [ReadOnly]](/components/Height){:target="_blank"}
  - [ProjectM.Velocity [ReadOnly]](/components/Velocity){:target="_blank"}
  - [ProjectM.BuffableFlagState [ReadOnly]](/components/BuffableFlagState){:target="_blank"}
  - [ProjectM.Shared.FallToHeight](/components/FallToHeight){:target="_blank"}
  - [Unity.Transforms.Translation](/components/Translation){:target="_blank"}
- **None Components:**
  - [ProjectM.CanFly [ReadOnly]](/components/CanFly){:target="_blank"}

### __query_496388984_4

- **All Components:**
  - [ProjectM.Height [ReadOnly]](/components/Height){:target="_blank"}
  - [ProjectM.Velocity [ReadOnly]](/components/Velocity){:target="_blank"}
  - [ProjectM.BuffableFlagState [ReadOnly]](/components/BuffableFlagState){:target="_blank"}
  - [ProjectM.CanFly](/components/CanFly){:target="_blank"}
  - [ProjectM.Shared.FallToHeight](/components/FallToHeight){:target="_blank"}
  - [Unity.Transforms.Translation](/components/Translation){:target="_blank"}

### __query_496388984_5

- **All Components:**
  - [Unity.Physics.PhysicsWorldSingleton [ReadOnly]](/components/PhysicsWorldSingleton){:target="_blank"}

### __query_496388984_6

- **All Components:**
  - [ProjectM.Tiles.TileWorldSingleton [ReadOnly]](/components/TileWorldSingleton){:target="_blank"}

### __query_496388984_7

- **All Components:**
  - [ProjectM.CurveCollection [ReadOnly]](/components/CurveCollection){:target="_blank"}

