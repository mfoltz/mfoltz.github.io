---
title: NetworkInterpolationSystem_Client
nav_exclude: true
search_exclude: true
---

# NetworkInterpolationSystem_Client (Client)

### __query_1748990320_0

- **All Components:**
  - [ProjectM.EntityInput](/components/EntityInput){:target="_blank"}
  - [ProjectM.Network.NetworkInterpolated_Shared [ReadOnly]](/components/NetworkInterpolated_Shared){:target="_blank"}
  - [ProjectM.Network.NetworkInterpolated_Client](/components/NetworkInterpolated_Client){:target="_blank"}
  - [ProjectM.LastTranslation](/components/LastTranslation){:target="_blank"}
  - [ProjectM.Velocity](/components/Velocity){:target="_blank"}
  - [Unity.Transforms.Rotation](/components/Rotation){:target="_blank"}
  - [Unity.Transforms.Translation](/components/Translation){:target="_blank"}
- **None Components:**
  - [ProjectM.Network.LocalControlled [ReadOnly]](/components/LocalControlled){:target="_blank"}
  - [Unity.Entities.DestroyTag [ReadOnly]](/components/DestroyTag){:target="_blank"}

### __query_1748990320_1

- **All Components:**
  - [ProjectM.Network.DisableNetworkInterpolation [ReadOnly]](/components/DisableNetworkInterpolation){:target="_blank"}

