---
title: PushMeshDataSystem
nav_exclude: true
search_exclude: true
---

# PushMeshDataSystem (Client)

### m_LayoutDeformedMeshesQuery

- **All Components:**
  - [Unity.Rendering.DeformedEntity [ReadOnly]](/components/DeformedEntity){:target="_blank"}
  - [Unity.Rendering.DeformedMeshIndex](/components/DeformedMeshIndex){:target="_blank"}
  - [Unity.Rendering.MaterialMeshInfo [ReadOnly]](/components/MaterialMeshInfo){:target="_blank"}
- **Any Components:**
  - [Unity.Rendering.BlendWeightBufferIndex](/components/BlendWeightBufferIndex){:target="_blank"}
  - [Unity.Rendering.SkinMatrixBufferIndex](/components/SkinMatrixBufferIndex){:target="_blank"}

### __query_1205703448_0

- **All Components:**
  - [Unity.Rendering.DeformedEntity [ReadOnly]](/components/DeformedEntity){:target="_blank"}
  - [Unity.Rendering.SharedMeshTracker [ReadOnly]](/components/SharedMeshTracker){:target="_blank"}
  - [Unity.Entities.Disabled [ReadOnly]](/components/Disabled){:target="_blank"}

### __query_1205703448_1

- **All Components:**
  - [Unity.Rendering.SharedMeshTracker [ReadOnly]](/components/SharedMeshTracker){:target="_blank"}
- **None Components:**
  - [Unity.Rendering.DeformedEntity [ReadOnly]](/components/DeformedEntity){:target="_blank"}

### __query_1205703448_2

- **All Components:**
  - [Unity.Rendering.DeformedEntity [ReadOnly]](/components/DeformedEntity){:target="_blank"}
  - [Unity.Rendering.MaterialMeshInfo [ReadOnly]](/components/MaterialMeshInfo){:target="_blank"}
- **None Components:**
  - [Unity.Rendering.SharedMeshTracker [ReadOnly]](/components/SharedMeshTracker){:target="_blank"}

