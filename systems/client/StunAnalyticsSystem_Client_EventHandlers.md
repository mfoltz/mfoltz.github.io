---
title: StunAnalyticsSystem_Client_EventHandlers
nav_exclude: true
search_exclude: false
---

# StunAnalyticsSystem_Client_EventHandlers (Client)

### _UserKillServerEventQuery

- **All Components:**
  - [ProjectM.Network.UserKillServerEvent [ReadOnly]](/components/UserKillServerEvent){:target="_blank"}

### _UserDownedServerEventQuery

- **All Components:**
  - [ProjectM.Network.UserDownedServerEvent [ReadOnly]](/components/UserDownedServerEvent){:target="_blank"}

### __query_1762304676_0

- **All Components:**
  - [ProjectM.Network.UserDownedServerEvent [ReadOnly]](/components/UserDownedServerEvent){:target="_blank"}

### __query_1762304676_1

- **All Components:**
  - [ProjectM.Network.UserKillServerEvent [ReadOnly]](/components/UserKillServerEvent){:target="_blank"}

### __query_1762304676_2

- **All Components:**
  - [ProjectM.AbilityPreCastFinishedEvent [ReadOnly]](/components/AbilityPreCastFinishedEvent){:target="_blank"}

### __query_1762304676_3

- **All Components:**
  - [ProjectM.AbilityInterruptedEvent [ReadOnly]](/components/AbilityInterruptedEvent){:target="_blank"}

