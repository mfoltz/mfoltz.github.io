---
title: ShowBuildGridSystem
nav_exclude: true
search_exclude: true
---

# ShowBuildGridSystem (Client)

### _NewPreviewQuery

- **All Components:**
  - [ProjectM.ShowBuildGrid](/components/ShowBuildGrid){:target="_blank"}
- **None Components:**
  - [ProjectM.DrawGrid [ReadOnly]](/components/DrawGrid){:target="_blank"}

### _CleanupPreviewQuery

- **All Components:**
  - [ProjectM.DrawGrid](/components/DrawGrid){:target="_blank"}
  - ProjectM.ShowBuildGridSystem+ShowBuildGridActive
- **None Components:**
  - [ProjectM.ShowBuildGrid [ReadOnly]](/components/ShowBuildGrid){:target="_blank"}

### _ActiveGridQuery

- **All Components:**
  - [ProjectM.DrawGrid](/components/DrawGrid){:target="_blank"}
  - [ProjectM.ShowBuildGrid [ReadOnly]](/components/ShowBuildGrid){:target="_blank"}
  - ProjectM.ShowBuildGridSystem+ShowBuildGridActive [ReadOnly]

### _ActivePreviewQuery

- **All Components:**
  - [ProjectM.IsPreviewingPlacement [ReadOnly]](/components/IsPreviewingPlacement){:target="_blank"}
  - [ProjectM.PreviewBuildingPlacement [ReadOnly]](/components/PreviewBuildingPlacement){:target="_blank"}

### __query_37087932_0

- **All Components:**
  - [ProjectM.EditableTileModel [ReadOnly]](/components/EditableTileModel){:target="_blank"}

## Invalid Queries

- _LocalUserControllerQuery
