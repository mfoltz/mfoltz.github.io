---
title: PlaceTileModelSystem
nav_exclude: true
search_exclude: false
---

# PlaceTileModelSystem (Server)

### _BuildTileQuery

- **All Components:**
  - [ProjectM.Network.FromCharacter [ReadOnly]](/components/FromCharacter){:target="_blank"}
  - [ProjectM.BuildTileModelEvent [ReadOnly]](/components/BuildTileModelEvent){:target="_blank"}

### _StartEditQuery

- **All Components:**
  - [ProjectM.Network.FromCharacter [ReadOnly]](/components/FromCharacter){:target="_blank"}
  - [ProjectM.StartEditTileModelEvent [ReadOnly]](/components/StartEditTileModelEvent){:target="_blank"}

### _CancelEditQuery

- **All Components:**
  - [ProjectM.Network.FromCharacter [ReadOnly]](/components/FromCharacter){:target="_blank"}
  - [ProjectM.CancelEditTileModelEvent [ReadOnly]](/components/CancelEditTileModelEvent){:target="_blank"}

### _MoveTileQuery

- **All Components:**
  - [ProjectM.Network.FromCharacter [ReadOnly]](/components/FromCharacter){:target="_blank"}
  - [ProjectM.MoveTileModelEvent [ReadOnly]](/components/MoveTileModelEvent){:target="_blank"}

### _DismantleTileQuery

- **All Components:**
  - [ProjectM.Network.FromCharacter [ReadOnly]](/components/FromCharacter){:target="_blank"}
  - [ProjectM.DismantleTileModelEvent [ReadOnly]](/components/DismantleTileModelEvent){:target="_blank"}

### _ConnectionChangedQuery

- **All Components:**
  - [ProjectM.UserConnectionChangedEvent [ReadOnly]](/components/UserConnectionChangedEvent){:target="_blank"}

### _AbilityCastFinishedQuery

- **All Components:**
  - [ProjectM.AbilityPreCastFinishedEvent [ReadOnly]](/components/AbilityPreCastFinishedEvent){:target="_blank"}

### _RepairTileQuery

- **All Components:**
  - [ProjectM.Network.FromCharacter [ReadOnly]](/components/FromCharacter){:target="_blank"}
  - [ProjectM.RepairTileModelEvent [ReadOnly]](/components/RepairTileModelEvent){:target="_blank"}

### _BuildWallpaperQuery

- **All Components:**
  - [ProjectM.Network.FromCharacter [ReadOnly]](/components/FromCharacter){:target="_blank"}
  - [ProjectM.BuildWallpaperEvent [ReadOnly]](/components/BuildWallpaperEvent){:target="_blank"}

### _SetVariationQuery

- **All Components:**
  - [ProjectM.Network.FromCharacter [ReadOnly]](/components/FromCharacter){:target="_blank"}
  - [ProjectM.SetTileModelVariationEvent [ReadOnly]](/components/SetTileModelVariationEvent){:target="_blank"}

### __query_112767830_0

- **All Components:**
  - [Unity.Physics.PhysicsWorldSingleton [ReadOnly]](/components/PhysicsWorldSingleton){:target="_blank"}

### __query_112767830_1

- **All Components:**
  - ProjectM.Network.NetworkIdSystem+Singleton [ReadOnly]

### __query_112767830_2

- **All Components:**
  - [ProjectM.Tiles.TileWorldSingleton [ReadOnly]](/components/TileWorldSingleton){:target="_blank"}

### __query_112767830_3

- **All Components:**
  - [ProjectM.ServerTime [ReadOnly]](/components/ServerTime){:target="_blank"}

### __query_112767830_4

- **All Components:**
  - [Stunlock.Core.PrefabLookupMap [ReadOnly]](/components/PrefabLookupMap){:target="_blank"}

### __query_112767830_5

- **All Components:**
  - [ProjectM.MapZoneCollection [ReadOnly]](/components/MapZoneCollection){:target="_blank"}

### __query_112767830_6

- **All Components:**
  - [ProjectM.Network.UserActivityGrid [ReadOnly]](/components/UserActivityGrid){:target="_blank"}

