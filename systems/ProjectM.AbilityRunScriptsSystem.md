---
title: ProjectM.AbilityRunScriptsSystem
nav_exclude: true
search_exclude: true
---

# ProjectM.AbilityRunScriptsSystem (Server)

### _OnCastStartedQuery

- **All Components:**
  - [ProjectM.AbilityCastStartedEvent [ReadOnly]](/components/AbilityCastStartedEvent){:target="_blank"}

### _OnPreCastFinishedQuery

- **All Components:**
  - [ProjectM.AbilityPreCastFinishedEvent [ReadOnly]](/components/AbilityPreCastFinishedEvent){:target="_blank"}

### _OnPreCastEndedQuery

- **All Components:**
  - [ProjectM.AbilityPreCastEndedEvent [ReadOnly]](/components/AbilityPreCastEndedEvent){:target="_blank"}

### _OnPostCastFinishedQuery

- **All Components:**
  - [ProjectM.AbilityPostCastFinishedEvent [ReadOnly]](/components/AbilityPostCastFinishedEvent){:target="_blank"}

### _OnPostCastEndedQuery

- **All Components:**
  - [ProjectM.AbilityPostCastEndedEvent [ReadOnly]](/components/AbilityPostCastEndedEvent){:target="_blank"}

### _OnCastEndedQuery

- **All Components:**
  - [ProjectM.AbilityCastEndedEvent [ReadOnly]](/components/AbilityCastEndedEvent){:target="_blank"}

### _OnInterruptedQuery

- **All Components:**
  - [ProjectM.AbilityInterruptedEvent [ReadOnly]](/components/AbilityInterruptedEvent){:target="_blank"}

### __query_2147175953_0

- **All Components:**
  - [ProjectM.AbilityCastStartedEvent [ReadOnly]](/components/AbilityCastStartedEvent){:target="_blank"}

### __query_2147175953_1

- **All Components:**
  - [ProjectM.AbilityPreCastFinishedEvent [ReadOnly]](/components/AbilityPreCastFinishedEvent){:target="_blank"}

### __query_2147175953_2

- **All Components:**
  - [ProjectM.AbilityPreCastEndedEvent [ReadOnly]](/components/AbilityPreCastEndedEvent){:target="_blank"}

### __query_2147175953_3

- **All Components:**
  - [ProjectM.AbilityPostCastFinishedEvent [ReadOnly]](/components/AbilityPostCastFinishedEvent){:target="_blank"}

### __query_2147175953_4

- **All Components:**
  - [ProjectM.AbilityPostCastEndedEvent [ReadOnly]](/components/AbilityPostCastEndedEvent){:target="_blank"}

### __query_2147175953_5

- **All Components:**
  - [ProjectM.AbilityCastEndedEvent [ReadOnly]](/components/AbilityCastEndedEvent){:target="_blank"}

### __query_2147175953_6

- **All Components:**
  - [ProjectM.AbilityInterruptedEvent [ReadOnly]](/components/AbilityInterruptedEvent){:target="_blank"}

### __query_2147175953_7

- **All Components:**
  - [ProjectM.GameDataInitializedSingleton [ReadOnly]](/components/GameDataInitializedSingleton){:target="_blank"}

### __query_2147175953_8

- **All Components:**
  - ProjectM.CurrentCastsSystem+Singleton [ReadOnly]

## Invalid Queries

- _RunScriptOnCastStartedQuery
- _RunScriptOnCastUpdateQuery
- _RunScriptOnPreCastFinishedQuery
- _RunScriptOnPreCastEndedQuery
- _RunScriptOnPostCastFinishedQuery
- _RunScriptOnPostCastEndedQuery
- _RunScriptOnCastEndedQuery
- _RunScriptOnInterruptedQuery
