---
title: ProjectM.ServantCoffinstationUpdateSystem
nav_exclude: true
search_exclude: true
---

# ProjectM.ServantCoffinstationUpdateSystem (Server)

### _ChangeServantNameEventQuery

- **All Components:**
  - [ProjectM.Network.FromCharacter [ReadOnly]](/components/FromCharacter){:target="_blank"}
  - [ProjectM.Network.ChangeServantNameEvent [ReadOnly]](/components/ChangeServantNameEvent){:target="_blank"}

### _CoffinStationQuery

- **All Components:**
  - [ProjectM.ServantCoffinstation](/components/ServantCoffinstation){:target="_blank"}
  - [ProjectM.EditableTileModel [ReadOnly]](/components/EditableTileModel){:target="_blank"}
  - [ProjectM.CastleWorkstation [ReadOnly]](/components/CastleWorkstation){:target="_blank"}

### __query_1849383143_0

- **All Components:**
  - [ProjectM.ServantCoffinstation [ReadOnly]](/components/ServantCoffinstation){:target="_blank"}
  - [ProjectM.EditableTileModel [ReadOnly]](/components/EditableTileModel){:target="_blank"}
  - [ProjectM.CastleWorkstation [ReadOnly]](/components/CastleWorkstation){:target="_blank"}

### __query_1849383143_1

- **All Components:**
  - [ProjectM.Network.FromCharacter [ReadOnly]](/components/FromCharacter){:target="_blank"}
  - [ProjectM.Network.ChangeServantNameEvent [ReadOnly]](/components/ChangeServantNameEvent){:target="_blank"}

### __query_1849383143_2

- **All Components:**
  - [ProjectM.ServantCoffinstation](/components/ServantCoffinstation){:target="_blank"}
  - [ProjectM.EditableTileModel [ReadOnly]](/components/EditableTileModel){:target="_blank"}
  - [ProjectM.CastleWorkstation [ReadOnly]](/components/CastleWorkstation){:target="_blank"}

## Invalid Queries

- _ActiveMissionsQuery
