---
title: ProjectM.CheckInSunSystem
nav_exclude: true
search_exclude: true
---

# ProjectM.CheckInSunSystem (Server)

### _DynamicCloudVolumeQuery

- **All Components:**
  - ProjectM.DynamicCloudVolume [ReadOnly]
  - [Unity.Transforms.LocalToWorld [ReadOnly]](/components/LocalToWorld){:target="_blank"}

### _RaycastJobQuery

- **All Components:**
  - [ProjectM.TakeDamageInSun](/components/TakeDamageInSun){:target="_blank"}
  - [Unity.Transforms.Translation [ReadOnly]](/components/Translation){:target="_blank"}
- **None Components:**
  - [ProjectM.Dead [ReadOnly]](/components/Dead){:target="_blank"}

### __query_1202091801_0

- **All Components:**
  - [ProjectM.TakeDamageInSun](/components/TakeDamageInSun){:target="_blank"}

### __query_1202091801_1

- **All Components:**
  - [ProjectM.TakeDamageInSun](/components/TakeDamageInSun){:target="_blank"}
  - [Unity.Transforms.Translation [ReadOnly]](/components/Translation){:target="_blank"}
  - [ProjectM.BuffBuffer [Buffer] [ReadOnly]](/components/BuffBuffer){:target="_blank"}
- **None Components:**
  - [ProjectM.Network.IsConnected [ReadOnly]](/components/IsConnected){:target="_blank"}

### __query_1202091801_2

- **All Components:**
  - [ProjectM.CloudCookie [ReadOnly]](/components/CloudCookie){:target="_blank"}
  - [Unity.Transforms.LocalToWorld [ReadOnly]](/components/LocalToWorld){:target="_blank"}
- **None Components:**
  - [ProjectM.CloudLight [ReadOnly]](/components/CloudLight){:target="_blank"}

### __query_1202091801_3

- **All Components:**
  - [ProjectM.CloudCookie [ReadOnly]](/components/CloudCookie){:target="_blank"}
  - [Unity.Transforms.LocalToWorld [ReadOnly]](/components/LocalToWorld){:target="_blank"}
  - [ProjectM.CloudLight [ReadOnly]](/components/CloudLight){:target="_blank"}

### __query_1202091801_4

- **All Components:**
  - [ProjectM.TakeDamageInSun](/components/TakeDamageInSun){:target="_blank"}
  - [ProjectM.BuffableFlagState [ReadOnly]](/components/BuffableFlagState){:target="_blank"}
  - [Unity.Transforms.Translation [ReadOnly]](/components/Translation){:target="_blank"}

### __query_1202091801_5

- **All Components:**
  - [ProjectM.SunBlocker.SunBlockerRegion [ReadOnly]](/components/SunBlockerRegion){:target="_blank"}

### __query_1202091801_6

- **All Components:**
  - [ProjectM.DayNightCycle [ReadOnly]](/components/DayNightCycle){:target="_blank"}

### __query_1202091801_7

- **All Components:**
  - [ProjectM.CastleBuilding.CastleBlockCollection [ReadOnly]](/components/CastleBlockCollection){:target="_blank"}

### __query_1202091801_8

- **All Components:**
  - [ProjectM.Sun [ReadOnly]](/components/Sun){:target="_blank"}

### __query_1202091801_9

- **All Components:**
  - [Unity.Physics.PhysicsWorldSingleton [ReadOnly]](/components/PhysicsWorldSingleton){:target="_blank"}

